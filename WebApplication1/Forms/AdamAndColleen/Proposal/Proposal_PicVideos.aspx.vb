...
Public Class Proposal_PicVideos
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'Get Seminar session and find directory
        'If (Not IsNothing(Page.Request.QueryString.Item("session"))) Then
        '    strPhotoDir &= "Seminar" & Page.Request.QueryString.Item("session") & "/"
        'End If
    ...
    'Private Sub FormatArrowsAndText()
    '    'Set left arrows
    '    topLeftArrow.Attributes("href") = "?nStart=" & (nStart - nPicsPerPage) & "&session=" & Page.Request.QueryString.Item("session")
    '    bottomLeftArrow.Attributes("href") = topLeftArrow.Attributes("href")
    '    If ((nStart - nPicsPerPage) < 0 Or dtPics.Rows.Count = 0) Then
    '        topLeftArrow.Style.Item("visibility") = "hidden"
    '        bottomLeftArrow.Style.Item("visibility") = "hidden"
    '    End If

    '    'Set right arrows
    '    topRightArrow.Attributes("href") = "?nStart=" & (nStart + nPicsPerPage) & "&session=" & Page.Request.QueryString.Item("session")
    '    bottomRightArrow.Attributes("href") = topRightArrow.Attributes("href")
    '    If ((nStart + nPicsPerPage) >= dtPics.Rows.Count Or dtPics.Rows.Count = 0) Then
    '        topRightArrow.Style.Item("visibility") = "hidden"
    '        bottomRightArrow.Style.Item("visibility") = "hidden"
    '    End If

    '    'Set top label
    '    litFromToPictures.Text = "Session " & Page.Request.QueryString.Item("session") & " : " & (nStart + 1) & " - " & IIf(nStart + 50 < dtPics.Rows.Count, nStart + 50, dtPics.Rows.Count) & " of " & dtPics.Rows.Count
    'End Sub
